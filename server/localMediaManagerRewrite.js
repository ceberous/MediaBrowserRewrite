...
async function wPlay() {
	...
	if ( !FinalNowPlaying.three_percent ) { FinalNowPlaying.three_percent = 0; }
	...
	const R_NP_ShowName_BackupKey = R_Live_Genre_Base + "." + FinalNowPlaying.show_name;
...
function updateLastPlayedTime( wTime ) {
	return new Promise( async function( resolve , reject ) {
		try {
			if ( wTime ) { console.log( "wTime === " + wTime.toString() ); }
			else { console.log( "no wTIME !!!!" ); }
			if ( wTime !== null ) {
			...
			else { G_NOW_PLAYING.completed = true; }
